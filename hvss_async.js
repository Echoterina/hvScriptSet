 ...
 * Additional improvements by: Shata
 * License: MIT
 *
 * This version of script is completely asynchronous and will not delay the load of forum page.
 */
...
let hvScriptSet = {
    ...
    addMask: async function (opt) {
        ...
        async function getPosts() {
            ...
            let checkAccess = changedUsersId.length > 0 ? await getAccess(changedUsersId) : {};
            for (let _i in changedPosts) {
                if (changedPosts.hasOwnProperty(_i)) {
                    changedPosts[_i].username = await checkAccess[changedPosts[_i].userId].username;
                    changedPosts[_i].groupId = await checkAccess[changedPosts[_i].userId].groupId;
                    changedPosts[_i].groupTitle = await checkAccess[changedPosts[_i].userId].groupTitle;
                    changedPosts[_i].access = await checkAccess[changedPosts[_i].userId].access;

                    if (changedPosts[_i].changeList.avatar && await changedPosts[_i].access.common) {
                ...
                if (changedPosts.hasOwnProperty(_i) && await changedPosts[_i].access.extended) {
        ...
        async function getAccess(usersId) {
            let userInfo = await getUsersInfo(usersId);
            const forumName = getClearedForumName(FORUM.topic.forum_name);
            for (let id in await userInfo) {
        ...
        async function getUsersInfo(usersId) {
            ...
            if (usersIdStr) {
                var url = new URL('/api.php', window.location.origin),
                    params = {method: 'users.get', user_id: usersIdStr}
                ...
                for (let i in json.response.users) {
                    if (json.response.users.hasOwnProperty(i)) {
                        usersInfo[await json.response.users[i].user_id] = {
                            'userId': i,
                            'username': await json.response.users[i].username,
                            'groupId': await json.response.users[i].group_id,
                            'groupTitle': await json.response.users[i].group_title
                        };
                    }
                }
            }
